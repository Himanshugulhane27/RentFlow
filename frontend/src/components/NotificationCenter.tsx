...
export const NotificationCenter: React.FC = () => {
  ...
  const NotificationItem = ({ n }: { n: Notification }) => (
    <motion.div
      ...
      exit={{ opacity: 0, x: "100%", transition: springSnappy }}
      whileHover={{ x: 2 }}
      onClick={() => {
        markAsRead(n.id);
        navigate(n.route);
      ...
      className={cn(
        "relative group flex items-start gap-3 px-4 py-3 cursor-pointer transition-all duration-200 border-l-2 hover:bg-hsl(var(--surface-2)/0.5)",
        !n.read ? "border-hsl(var(--brand-500)) bg-hsl(var(--brand-50)/0.5)" : "border-transparent bg-transparent"
      )}
    >
      <div className={cn("w-9 h-9 rounded-[var(--radius-md)] flex items-center justify-center flex-shrink-0 mt-0.5", iconBgByType[n.type] || 'bg-hsl(var(--surface-3)) text-hsl(var(--text-secondary))')}>
      ...
      >
        ...
        {unreadCount > 0 && (
          <motion.span
            initial={{ scale: 0 }}
            animate={{ scale: 1 }}
            className="absolute top-1.5 right-1.5 w-2 h-2 bg-danger-500 border border-white rounded-full"
          />
        )}
      </button>
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="fixed bottom-0 inset-x-0 rounded-t-2xl sm:absolute sm:top-full sm:bottom-auto sm:inset-x-auto sm:right-0 sm:mt-2 w-full sm:w-96 max-h-[80vh] sm:max-h-[480px] overflow-hidden glass-medium elevation-4 sm:rounded-[var(--radius-xl)] z-50 flex flex-col"
          >
            <div className="flex items-center justify-between px-5 py-4 border-b border-hsl(var(--surface-border)/0.5) bg-transparent sticky top-0 z-10">
              <div className="flex items-center gap-2">
                <p className="text-sm font-semibold text-hsl(var(--text-primary))">Notifications</p>
                {unreadCount > 0 && (
                  <span className="bg-hsl(var(--brand-500)) text-white text-xs font-bold px-2 py-0.5 rounded-[var(--radius-full)]">
                    {unreadCount}
                  </span>
                )}
              </div>
              {unreadCount > 0 && (
                <button 
                  ...
                  className="text-xs text-hsl(var(--brand-500)) hover:text-hsl(var(--brand-600)) font-medium transition-colors"
                >
                  Mark all read
                </button>
              )}
              ...
              {isLoading ? (
                ...
                >
                  <AnimatePresence>
                    ...
                    <div className="py-2">
                      <p className="text-xs font-medium uppercase tracking-wider text-neutral-400 px-4 py-2">Urgent</p>
                      {urgentItems.map(n => <NotificationItem key={n.id} n={n} />)}
                    </div>
                  )}
                  {upcomingItems.length > 0 && (
                    <div className="py-2 border-t border-neutral-50">
                      <p className="text-xs font-medium uppercase tracking-wider text-neutral-400 px-4 py-2">Upcoming</p>
                      {upcomingItems.map(n => <NotificationItem key={n.id} n={n} />)}
                    </div>
                  )}
                  {recentItems.length > 0 && (
                    <div className="py-2 border-t border-neutral-50">
                      <p className="text-xs font-medium uppercase tracking-wider text-neutral-400 px-4 py-2">Recent</p>
