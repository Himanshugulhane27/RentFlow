...
export const FilterPanel: React.FC<FilterPanelProps> = ({
  ...
  const content = (
    <div className="flex flex-col gap-6">
          ...
          <div className="space-y-2">
            {(['paid', 'overdue', 'pending'] as const).map(s => (
              <label key={s} className="flex items-center gap-3 cursor-pointer group">
                <div className="relative flex items-center justify-center w-5 h-5 rounded-[var(--radius-sm)] border border-hsl(var(--surface-border)) bg-hsl(var(--surface-0)) group-hover:border-hsl(var(--brand-400)) transition-colors">
                  <input 
                    type="checkbox" 
                    className="sr-only" 
      ...
      <div className="pt-4 border-t border-hsl(var(--surface-border)) mt-2">
  ...
  return (
    <>
      <div className="hidden lg:block w-64 flex-shrink-0">
        <div className="bg-hsl(var(--surface-0)) elevation-2 card-border rounded-[var(--radius-lg)] p-5 sticky top-6">
          {content}
        </div>
      </div>
        ...
        {isOpenMobile && (
          <div className="fixed inset-0 z-50 flex flex-col justify-end lg:hidden">
            ...
            <motion.div 
              variants={slideUp}
              initial="initial" animate="animate" exit="exit"
              className="relative bg-hsl(var(--surface-0)) rounded-t-[var(--radius-xl)] p-5 max-h-[85vh] overflow-y-auto w-full"
            >
              <div className="flex justify-end mb-2">
                <button onClick={() => setIsOpenMobile(false)} className="p-2 text-hsl(var(--text-tertiary))"><X size={20}/></button>
              </div>
              {content}
            </motion.div>
          </div>
